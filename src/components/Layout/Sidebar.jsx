    ...
    <div className={`${collapsed ? "w-20" : "w-72"} transition-all duration-300 ease-in-out bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-300/60 dark:border-slate-700/50 flex flex-col relative z-10`}>
      ...
      <nav className='flex-1 p-4 space-y-2 overflow-y-auto'>
        {menuItems.map((item)=>{
          return (
          <div key={item.id}>
            <button className={`w-full flex items-center justify-between p-3 rounded-xl transition-all duration-200 dark:text-white ${currentPage === item.id || item.active ? "bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg shadow-blue-500/25 " : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/50"}`} onClick={()=>{
              if(item.submenu){
            ...
            >
              <div className='flex items-center space-x-3'><item.icon className={'w-5 h-5'} />
              {/* Conditional Rendering */}
              ...
              {!collapsed && item.submenu && (<ChevronDown className={`w-4 h-4 transition-transform`} />)}
                  ...
                  {!collapsed && item.submenu && expandedItems.has(item.id) && (
                    ...
                    {item.submenu.map((subitem)=>{
                      return <button key={subitem.id} className='w-full text-left p-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/50 rounded-lg transition-all'>{subitem.label}</button>
                    })}
                  </div>
                  )}
          </div>
      ...
      {/* User Profile */}
      {!collapsed && (
        <div className='p-4 border-t border-slate-300/60 dark:border-slate-700/50'>
      <div className='flex items-center space-x-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50'>
        <img src="https://images.pexels.com/photos/1681010/pexels-photo-1681010.jpeg?auto=compress&cs=tinysrgb5w=64&h=64&dpr=2" alt="user" className=' rounded-full w-10 h-10 ring-2 ring-blue-500'/>
