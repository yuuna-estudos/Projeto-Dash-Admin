...
function Header({ sideBarCollapsed, onToggleSidebar, isDarkMode, toggleTheme, searchTerm, setSearchTerm }) {
  ...
  {/* Efeito para buscar na API sempre que o termo debounce mudar */}
  useEffect(() => {
  ...
  const searchBarContent = (
    <div className='relative w-full' ref={dropdownRef}>
      ...
      {/* Ícone de Loading dentro do input */}
      ...
      {showDropdown && (debouncedGlobalTerm.length > 1) && (
        <div className='absolute top-full left-0 right-0 mt-2  border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden z-[100] max-h-[400px] overflow-y-auto backdrop-blur-xl bg-white/95 dark:bg-slate-900/95'>
          ...
          <div className='p-2'>
            ...
            {searchResults.users.length > 0 ? searchResults.users.map(user => (
              <div key={user.id} className='flex items-center space-x-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer rounded-xl transition-colors group'>
          ...
          <div className='p-2'>
            <h3 className='px-3 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider'>
              ...
              {searchResults.products.length > 0 ? searchResults.products.map(product => (
                <div key={product.id} className='flex items-center space-x-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer rounded-xl transition-colors group'>
